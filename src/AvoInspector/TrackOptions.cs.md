...
Optional per-call gateway coordinates for `AvoInspector.TrackSchemaFromEvent` (AVO-3516). Used when
this SDK's Inspector API key is a **gateway** key shared across destinations, to label where an
observation was taken: which output it was bound for, and optionally which upstream source
produced it. Plain mutable POCO with public get/set properties, all defaulting to `null`.
...
- Without `OriginHint`, a non-blank `AppVersion` (trimmed) overrides the instance's configured
  ...
  version, unchanged.
- Not part of `avohq/spec-first-inspector-server-sdk` v1.0.0 — this is an Avo-contract extension
  (AVO-3516 / AVO-3543), documented the same way the `sessionId` divergence is documented on
  `Internal.WireEvent`.
