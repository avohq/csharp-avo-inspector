...
- **Sends the SPEC.md §7.2 request headers:** `api-key` (from `apiKey`) and `env` (from `env`) — `/inspector/v2/track` REQUIRES both and answers `400` when either is missing or invalid — plus `X-Avo-Client`, whose value is `InspectorVersion.LibPlatform` (`"csharp"`), the token the Inspector edge attributes traffic by. **IMPORTANT: added with `TryAddWithoutValidation`, not `Add`.** `Add` throws `FormatException` on a pathological header value, and it is called outside the try/catch below, which would break the "never throws" contract. Skipping the up-front validation is safe: the HTTP stack still rejects a CR/LF-bearing header value when it writes the request, and that throw lands in the catch as `SendStatus.Error`.
